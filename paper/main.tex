...
values are cross-checked but have not been reduced to proof objects. The
fifth value was computed alongside the others but was withheld from an
earlier draft of this paper because one of our own validation gates for its
family had been started and killed without a verdict; it is claimed here only
because that gate has since been run to completion, and we record the episode
because the policy is the point. All code, certificates, proof-object tooling
and result records are public.
\end{abstract}
...
The fifth value was computed by the same method and at the same time as the
others, but was withheld from an earlier draft of this paper: the validation
gate that reproduces the published adjacent term of its family as an
independent check on the method had been started and killed without a
verdict, and our discipline does not claim an extension past an unfinished
gate. That gate has since been run to completion and passed;
Section~\ref{sec:a217059} records the value together with the history.
...
an extension is claimed. The value was therefore withheld: excluded from an
earlier draft of this paper, marked withheld in the public repository, and
not submitted to the OEIS. The gate was run to completion on 2026-08-11 and
...
answer looks right; this subsection exists because the gate ran, not because
