% Five new mixed van der Waerden numbers
...
\title{New terms for five OEIS sequences\\of mixed van der Waerden numbers}
\author{Leo Y. Zhang\thanks{Corresponding author. ORCID: \url{https://orcid.org/0009-0008-1145-7969}}\\[2pt]
  \small Independent researcher, United Kingdom\\[2pt]
  \small \href{mailto:academic.submissions26@gmail.com}{\texttt{academic.submissions26@gmail.com}}}
\date{September 2026}
...
\begin{abstract}
We determine the next term of each of five sequences of mixed van der Waerden
numbers: $w(14;\,2^{12},3,4)=57$, $w(21;\,2^{19},3,3)=52$,
$w(9;\,2^{7},4,4)=68$, $w(6;\,2^{4},4,5)=84$ and $w(11;\,2^{9},3,5)=74$. Each
extends by one term a sequence in the On-Line Encyclopedia of Integer
Sequences (\seq{A217058}, \seq{A217005}, \seq{A217007}, \seq{A217236},
\seq{A217059}) whose published values, due to Ahmed, had stood since 2012 or
2013.

What the paper establishes beyond the five values is the standard of proof.
Every upper bound is a SAT refutation obtained by cube-and-conquer and reduced
to formally checked proof objects: $23{,}851$ per-cube DRAT proofs in total,
each replayed to \texttt{s VERIFIED} by \texttt{drat-trim} against a single
formula, and for each family a checked proof that the cube set is exhaustive.
In every certified refutation the symmetry breaking is switched off, so what
is certified is the raw encoding rather than a reduced one. Each refutation
was additionally re-derived in a configuration carrying no reversal-symmetry
constraint, which agreed in every case. Every lower bound ships as an explicit
colouring, checkable against the definition in milliseconds by a standalone
program that uses only the Python standard library and never invokes a SAT
solver. No bound here rests on a solver's unsupported verdict.

Reaching full certification for the two families whose two targets are equal
required removing a colour-symmetry rule from the cube generator: it discards
colour-permutation images, which is sound when searching but is not a
refutation, and the exhaustiveness check correctly refuses to close without
it. For every family, the validation gate that reproduces the published
adjacent term was run to completion before the extension was claimed. All
code, certificates, proof-object tooling and result records are public.
\end{abstract}
...
\cite{ahmed2009,ahmed2010,ahmed2011,ahmed2013} and entered several families into the On-Line
Encyclopedia of Integer Sequences in 2012 \cite{oeis}, adding one further term to
\seq{A217059} in March 2013. The five families
treated here --- \seq{A217058} $=w(j+2;\,2^j,3,4)$, \seq{A217005}
$=w(j+2;\,2^j,3,3)$, \seq{A217007} $=w(j+2;\,2^j,4,4)$, \seq{A217236}
$=w(j+2;\,2^j,4,5)$ and \seq{A217059} $=w(j+2;\,2^j,3,5)$ --- had received no
new terms since then.
...
\midrule
\seq{A217058} & $a(12) = w(14;\,2^{12},3,4)$ & $57$ & approved by the OEIS, 6 August 2026 \\
\seq{A217005} & $a(19) = w(21;\,2^{19},3,3)$ & $52$ & approved by the OEIS, 12 August 2026 \\
\seq{A217007} & $a(7) \;\,= w(9;\,2^{7},4,4)$ & $68$ & approved by the OEIS, 12 August 2026 \\
\seq{A217236} & $a(4) \;\,= w(6;\,2^{4},4,5)$ & $84$ & approved by the OEIS, 12 August 2026 \\
\seq{A217059} & $a(9) \;\,= w(11;\,2^{9},3,5)$ & $74$ & approved by the OEIS, 13 August 2026 \\
\bottomrule
\end{tabular}
\end{center}

Each family carries a validation gate: the published adjacent term is
re-derived by the same machinery, as an independent check on the method in
that family, before an extension is claimed. For \seq{A217059} that gate was
finished after the computation rather than before it, so the value was held
back until it had run; Section~\ref{sec:a217059} gives the timings and the
verdicts.
...
(Section~\ref{sec:verification}); and the second, for all five values, by reducing
each refutation to DRAT proof objects checked by an independently built
verifier, with the symmetry breaking switched off so that what is certified
is the raw encoding (Section~\ref{sec:drat}).

We are explicit throughout about which claims carry which kind of evidence.
For the upper bounds the answer is now uniform: each of the five rests on
per-cube DRAT proofs replayed to \texttt{s VERIFIED}, together with a checked
composition proof that the cube set covers the whole assignment tree. What
remains trusted is \texttt{drat-trim} itself and the claim that the CNF says
what the definition says, and the second of those is what the exhaustive
encoding audit of Section~\ref{sec:verification} addresses.
...
\]
A colouring \emph{with wildcards} assigns each integer either to one of the
$r$ real classes or to a wildcard position that belongs to no class; it is
valid for $(t_1,\dots,t_r)$ if class $i$ contains no $t_i$-term arithmetic
progression, for every $i$. In certificates below, \texttt{.}\ denotes a
wildcard and \texttt{1}, \texttt{2} the two real classes. Throughout, a
colouring is a map into the set of classes and is not required to be
surjective: classes may be empty.

The encoding of Section~\ref{sec:encoding} decides the wildcard formulation,
not the definition above, so we record the bridge between them.

\begin{lemma}\label{lem:wildcards}
For all $j \ge 0$ and all $t_1,\dots,t_r \ge 2$,
\[
...
\begin{proof}
Fix $n$. We show that $\{1,\dots,n\}$ admits a valid $(j+r)$-colouring for
$(2^j,t_1,\dots,t_r)$ if and only if it admits a valid colouring with at most
$j$ wildcards.

($\Rightarrow$) Let $c$ be such a colouring. Any two distinct integers form a
$2$-term arithmetic progression, so each of the $j$ classes of target $2$
holds at most one element and their union has at most $j$ elements. Send every
element of that union to a wildcard and leave the other $r$ classes as they
are. No class of target $t_i$ has changed, so the result is valid and uses at
most $j$ wildcards.

($\Leftarrow$) Let $d$ be valid with $m \le j$ wildcards. Assign the $m$
wildcard positions to $m$ distinct classes of target $2$, one element each,
and leave the remaining $j-m$ such classes empty, which the convention above
permits. Every class of target $2$ then holds at most one element and so
contains no $2$-term progression, and the $r$ other classes are unchanged, so
the result is a valid $(j+r)$-colouring.

Restricting a valid colouring of $\{1,\dots,n\}$ to $\{1,\dots,n-1\}$ leaves
it valid, so the set of $n$ admitting one is an initial segment of the
positive integers. Its supremum is therefore one less than the least $n$
admitting none, which is the displayed identity.
\end{proof}
...
free bound is tight varies by family, and its role is reported in
...
verification programme of Sections~\ref{sec:verification}
and~\ref{sec:drat} is arranged so that it is never assumed by its own check.

\subsection{Cube-and-conquer and the completeness of the cube set}\label{sec:cube}
...
the residual formulas in parallel. The same division of labour --- separate
...
an error rather than being counted as an empty branch. That failure mode was observed
during development, which is why the rule is enforced in code rather than
assumed.

\section{The five new terms}\label{sec:results}
...
wildcards exists. The refutation was established in four configurations,
varying the symmetry breaking, the solver and the cube depth:
...
The second row is the one that matters: \texttt{vdw2}, the earlier
engine from which \texttt{vdw4} was derived, imposes no lex-leader constraint
on this family and searches the full unreduced space, so it cannot inherit an
error from the reversal-symmetry constraint. It agrees. It shares its
encoding core with \texttt{vdw4} and is therefore not an independent
implementation; what it isolates is that one constraint, and nothing further. The third row was started and
...
verified a witness of its own at $n=51$ ($1934.0$\,s). This refutation is
also reduced to checked proof objects: $3{,}347$ per-cube DRAT proofs and a
composition proof, all replayed to \texttt{s VERIFIED}
(Section~\ref{sec:drat}, Table~\ref{tab:certified}).
...
full-scale gate passed before the extension was claimed: published
...
A qualification: this certificate \emph{is} the free construction of
...
$7285.2$\,s. It is also reduced to checked proof objects: $5{,}730$ per-cube
DRAT proofs and a composition proof, all replayed to \texttt{s VERIFIED}
(Table~\ref{tab:certified}). Being an equal-target family, this is one of the
two that the colour-symmetry rule of Section~\ref{sec:drat} blocked until it
was removed from the cube generator. The family gate passed before the extension was claimed:
published $a(6)=67$ was reproduced
...
the term. Monotonicity (Section~\ref{sec:mono}) is what licenses this. It is also
reduced to checked proof objects: $5{,}486$ per-cube DRAT proofs and a
composition proof, all replayed to \texttt{s VERIFIED}
(Table~\ref{tab:certified}). This is the one family whose exhaustiveness walk
relies substantially on unit propagation rather than on single falsified
clauses, for the reason given in Section~\ref{sec:drat}. The
...
\subsection{$\seq{A217059}$: $w(11;\,2^{9},3,5)=74$}\label{sec:a217059}

Class~\texttt{1} must avoid $3$-term progressions, class~\texttt{2} must
avoid $5$-term progressions. Published terms (offset $0$, OEIS keyword
\texttt{hard}, confirmed live against the OEIS API on 2026-07-29):
\[
a(0..8) = 22,\,32,\,43,\,44,\,50,\,55,\,61,\,65,\,70 .
\]

This value has a history the other four do not, which is why it appears
last. It was computed and cross-checked in July alongside the others, but
the family gate --- reproducing the published $a(8)=70$ as an independent
check on the method in this family --- had been started and killed without a
verdict, and our discipline requires that gate to run to completion before
an extension is claimed. The value was therefore not claimed: it was marked
withheld in the public repository and kept out of the OEIS until the gate had
finished. The gate was run to completion on 2026-08-11 and
passed: published $a(8)=70$ reproduced exactly --- \textsc{sat} at $n=69$
with a verified witness using all $8$ wildcards and \textsc{unsat} at
$n=70$, both at $j=8$, $978.8$\,s for the pair. A verification step that was
defined and then not finished does not get waived retroactively because the
answer looks right; the value is stated here because the gate ran, not because
the answer stopped being in doubt --- by the evidence below it never was.

\begin{theorem}\label{thm:a217059}
$a(9) = w(11;\,2^{9},3,5) = 74$.
\end{theorem}

\emph{Lower bound.} This colouring of $\{1,\dots,73\}$ uses exactly $9$
wildcards, with no $3$-term progression in class~\texttt{1} and no $5$-term
progression in class~\texttt{2}, so $a(9) > 73$:

\begin{center}\small
\texttt{21121222212222.22221122112..2.2222.2222.2122..2211221212222.2222121221122}
\end{center}

\noindent Class~\texttt{1} occupies $17$ positions, class~\texttt{2}
occupies $47$, and $9$ are wildcards. The bound was earned rather than
inherited: the free construction of Section~\ref{sec:mono} applied to
$a(8)=70$ gives only $a(9) \ge 71$, three short of the truth. Witnesses were
found by search and verified at $n=71$, $72$ and $73$ in turn ($1505.6$\,s,
$974.4$\,s, $3464.0$\,s). A \textsc{sat} answer to a probe launched hoping
for a refutation is easy to discard as a failed run; it is not --- each one
raises the floor, and three of them did most of the work of locating this
term.

\emph{Upper bound.} $n=74$, $j=9$ is unsatisfiable: $3859.9$\,s, all $76$
cubes reporting an explicit verdict. As in the previous family the value was
bracketed rather than climbed: a probe at $n=75$ refuted in $3484.7$\,s,
capping the value, while the witness at $n=73$ raised the floor to $74$,
leaving $n=74$ the single undecided instance. It is also reduced to checked
proof objects: $4{,}801$ per-cube DRAT proofs and a composition proof, all
replayed to \texttt{s VERIFIED} (Table~\ref{tab:certified}). The refutation
was established three ways:
...
encoding & symmetry & solver & cubes & verdict & time \\
\midrule
vdw4 & reversal on & CaDiCaL 1.9.5 & $k=4$ & \textsc{unsat} & $3859.9$\,s \\
vdw2 & none at all & CaDiCaL 1.9.5 & $k=4$ & \textsc{unsat} & $3452.9$\,s \\
vdw4 & reversal off & CaDiCaL 1.9.5 & $k=4$ & \textsc{unsat} & $3460.0$\,s \\
\bottomrule
\end{tabular}
\end{center}

\noindent The targets are distinct, so as in
Sections~\ref{sec:a217058} and~\ref{sec:a217236} the colour-swap breaker
emits no clauses and \texttt{vdw2} searches the full unreduced space.
\texttt{vdw2} also re-ran the satisfiable side at $n=73$ from scratch and
found a verified witness of its own in $2416.4$\,s --- and unlike the
previous family's, it is a \emph{different} colouring, diverging from the
one printed above at position $5$, so the satisfiability of $n=73$ is
certified by two independent objects.

\begin{remark}
$a(9)-a(8)=4$ against published differences $10,11,1,6,5,6,4,5$; a step of
$4$ already occurs at $a(6)=61 \to a(7)=65$. The differences of this family
are erratic --- an $11$ and a $1$ sit adjacent --- which is precisely why
the free bound was a poor predictor and the value had to be cornered rather
than extrapolated.
\end{remark}
...
adjacent index was run to completion before its extension was
\emph{claimed}; the gates for the other four families are reported in
Sections~\ref{sec:a217005}--\ref{sec:a217059}, and for \seq{A217059} that
gate ran after the computation rather than before it
(Section~\ref{sec:a217059}). For \seq{A217058} the gate
...
\subsection{Cross-checks against the reversal constraint}

Every established value's refutation was re-derived through \texttt{vdw2},
the earlier engine from which \texttt{vdw4} was derived, which carries no
reversal-symmetry constraint, with agreement in every case, as detailed per
family above. \texttt{vdw2} shares its progression enumeration, its clause
construction and its cardinality encoding with \texttt{vdw4}: it is not an
independent implementation, and a defect common to both would not be caught
by these runs. What they isolate is the reversal lex-leader constraint, the
one component \texttt{vdw4} adds. The stronger guarantee is the one in
Section~\ref{sec:drat}, where symmetry breaking is switched off altogether
and what is certified is the raw encoding. The records are the
\texttt{crosscheck\_*.json} files in the repository.

\section{Formally checked proof objects for all five
refutations}\label{sec:drat}
...
\subsection{Scope of the certification}
...
four families, are replayed under \texttt{drat-trim} on every full run of
the repository's verification gate and come back \texttt{s VERIFIED}.

\emph{All five} new upper bounds are certified by the cube-level route
described below: for each, every cube's refutation is replayed to
\texttt{s VERIFIED} and the cube set is proved exhaustive by a composition
proof that is itself replayed to \texttt{s VERIFIED}. Table~\ref{tab:certified}
records the sizes.
...
\midrule
\seq{A217058} & $w(14;\,2^{12},3,4)=57$ & $4{,}487$ & $3{,}236$ & $7{,}944$ \\
\seq{A217005} & $w(21;\,2^{19},3,3)=52$ & $3{,}347$ & $3{,}104$ & $6{,}482$ \\
\seq{A217059} & $w(11;\,2^{9},3,5)=74$  & $4{,}801$ & $3{,}222$ & $10{,}243$ \\
\seq{A217007} & $w(9;\,2^{7},4,4)=68$   & $5{,}730$ & $3{,}331$ & $10{,}171$ \\
\seq{A217236} & $w(6;\,2^{4},4,5)=84$   & $5{,}486$ & $3{,}545$ & $11{,}827$ \\
\bottomrule
\end{tabular}
\caption{Per-cube proof obligations and the composition proof that discharges
exhaustiveness. Every cube and every composition tail replays to
\texttt{s VERIFIED}.}
\label{tab:certified}
\end{table}
...
First, switching symmetry breaking off in the \emph{formula} is not
sufficient. The cube generator applied the same idea a second time, discarding
a prefix whenever two colours sharing a target value appeared out of canonical
order. That is sound when searching, since some other prefix is its
colour-permutation image, but it is not a refutation: the discarded prefix is
consistent with $F$, precisely because $F$ no longer carries the
symmetry-breaking clauses. The per-cube proofs therefore never covered those
branches, and the exhaustiveness check fails closed on them, naming an
uncovered prefix. The rule is a no-op whenever the targets are pairwise
distinct, which is why it went unnoticed while only \seq{A217058}, with
targets $3$ and $4$, was certified; it is exactly the two equal-target
families, \seq{A217005} and \seq{A217007}, that it blocked. Removing it
roughly doubles the cube set and changes nothing for the other three.

Second, \seq{A217236} is the family here for which $k$ exceeds $j$ by the most
(\seq{A217007}, with $k = 8 > j = 7$, invoked the fallback exactly once), and it is
the only one whose exhaustiveness walk makes substantial use of the unit-propagation
fallback ($351$ such prefixes, against $184$ refuted by a single falsified
clause). This is expected rather than anomalous: a prefix spending more than
$j$ wildcards violates the cardinality bound, and that bound is a totalizer
encoding, so it refutes by propagation across several clauses instead of
falsifying one. A propagation conflict is as sound a refutation as a falsified
clause, but the walk must retain the fallback for any instance with $k > j$.
...
reversal lex-leader constraint --- the one component the engine adds --- must
not be assumed by the thing meant to check it. What is
...
\subsection{The cube-level route in detail, on $n=57$, $j=12$}
...
of known values, not by exhaustion. The proof objects reduce what must be
trusted but do not eliminate it: \texttt{drat-trim} and the
encoding-equals-definition claim both remain inside the trusted base. All
computations ran on a single 8-core
desktop machine; per-instance solver times are reported so that the scale of
the computation is not overstated.

\section{Declarations}\label{sec:authorship}

\subsection*{Use of generative AI}

The author declares the following use of a generative AI system in the
preparation of this work: Anthropic's Claude, used through the Claude Code
command-line interface. The project ran from July to September 2026 and the
underlying model changed over that period as successive models of Anthropic's
Opus series were released, most recently Claude Opus~5; the series and the
period are given rather than a single version number, which would misstate a
project of this length.

\emph{Software and computation.} The SAT encodings, the verification and
proof-checking tooling, the DRAT pipeline and the test suites were written
with substantial assistance from that system, and the computations were run
and supervised with it. This is use of an AI system as a programming tool, on
the same footing as the SAT solver and the proof checker on which this paper
also depends. The resulting code is public, and its outputs are checked by
the independent means described below rather than accepted on the authority
of whatever produced them.

\emph{Text.} The manuscript was drafted and revised with assistance from the
same system. Every sentence in it has been reviewed, edited and approved by
the author.

\emph{Mathematics and accountability.} The research questions, the choice of
families and targets, the design of the verification programme, and every
claim made in this paper are the author's. The AI system is not an author, it
advances no claim of its own, and it bears no responsibility for the
contents. The author takes full responsibility for the whole of this work,
including those parts prepared with AI assistance.

This is stated plainly because the reader is entitled to weigh it, and
because the verification programme of
...
standard-library script that shares no code with the search; and every upper
bound, without exception, is a set of DRAT proofs replayed by an
independently built checker whose own failure modes were tested first,
together with a checked proof that those cubes cover the search. Nothing in
this paper asks to be believed on the strength of the process that produced
it.
...
Each family's proof-object certification is reproduced by
\texttt{vdw/cube\_certify.py} followed by \texttt{vdw/cube\_exhaustive.py},
both resumable; the recorded runs are the \texttt{vdw/cube\_run\_*}
directories, each holding its per-cube results, the run status and the
composition certificate. Times on an 8-core desktop ranged from under three
hours (\seq{A217005}, \seq{A217059}) to about five (\seq{A217236}).

One practical note, because it cost a run. Worker count is bounded by disk
throughput, not by cores or memory: \texttt{kissat} peaks near $33$\,MB and
\texttt{drat-trim} near $420$\,MB, but \seq{A217236} writes proofs of
$200$--$650$\,MB each, and fourteen concurrent workers writing and re-reading
those made every cube fail. Six workers verified the same cubes cleanly, and
twelve measured twice as slow as six on identical work.

Build notes for the two binaries, including two silent Windows-specific
pitfalls that produce wrong results rather than errors, are in
\texttt{vdw/DRAT.md}.
...
All code, certificates, proof-object tooling and result records supporting this
paper are openly available at \url{https://github.com/Leo-Y-Zhang/MathRecords}.
The exact snapshot on which every value in this paper was computed and checked
is permanently archived as version \texttt{v1.0.0} at
\url{https://doi.org/10.5281/zenodo.22801469}; that fixed version, rather than
the repository's changing head, is what the claims made here refer to.
...
\bibitem{ahmed2010}
T.~Ahmed,
\emph{Two new van der Waerden numbers: $w(2;3,17)$ and $w(2;3,18)$},
Integers \textbf{10} (2010), \#A32, 369--377.
