...
\title{Five new mixed van der Waerden numbers}
\author{Leo Y. Zhang}
\date{August 2026}
...
engine that applies no symmetry breaking at all. For the first of the five
...
that the cube set is exhaustive. The refutations behind the remaining four
values are cross-checked but have not been reduced to proof objects. For every
...
In particular: of the five new values, only the refutation behind
$w(14;\,2^{12},3,4)=57$ has been reduced to formally checked proof objects;
the other four rest on cross-checked solver verdicts, not on checked proofs.
...
$7285.2$\,s. The family gate passed before the extension was claimed:
...
leaving $n=74$ the single undecided instance. The refutation was established
three ways:
...
the repository's verification gate and come back \texttt{s VERIFIED}. The
headline refutation $n=57$, $j=12$ is certified by the cube-level route
described below. The refutations behind the other four new values ---
$w(21;\,2^{19},3,3)=52$, $w(9;\,2^{7},4,4)=68$, $w(6;\,2^{4},4,5)=84$ and
$w(11;\,2^{9},3,5)=74$ ---
have \emph{not} been reduced to proof objects; those upper bounds rest on
the cross-checked solver verdicts of
Sections~\ref{sec:a217005}--\ref{sec:a217059}, not on checked proofs.
...
of known values, not by exhaustion. Four of the five upper bounds are not
